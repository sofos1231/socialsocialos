...
export async function seedCatalog(prisma: PrismaService): Promise<void> {
  for (const item of CATALOG) {
    await prisma.shopCatalog.upsert({
      where: { key: item.key },
      update: { type: item.type, pricing: item.pricing, bonus: item['bonus'] ?? undefined, active: true },
      create: { key: item.key, type: item.type, pricing: item.pricing, bonus: item['bonus'] ?? undefined, active: true },
    });
  }
}
